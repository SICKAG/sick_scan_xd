...
:question: Independant of the configuration, the LMS1xxx pointcloud always displays 0.75° angular resolution

:white_check_mark: Using higher resolutions, the LMS1xxx sends scan data interlaced. With configuration ang_res=0.75, the angular resolution of each scan is 0.75°. This means that each point cloud message also has a resolution of 0.75°. With configuration ang_res=0.375, the scan is generated interlaced: Each scan still has 0.75° resolution, but 2 consecutive scans are rotated by 0.375° against each other. I.e. 2 consecutive Pointcloud messages each have an angular resolution of 0.375° at half the frequency. Within a point cloud message the angular resolution is still 0.75°.

With configuration ang_res=0.1875 the scan is generated quadruple interlaced, i.e. 4 consecutive scans are each rotated by 0.1875°. Each scan is resolved with 0.75°; 4 scans or 4 pointclouds together (accumulated) result in a resolution of 0.1875° at a quarter of the frequency.
...
The angular resolution is (just roughly measured) about atan(0.11/0.9) / 9 points = 0.77°. With ang_res=0.375 and decay=0.1 rviz shows twice the resolution:
