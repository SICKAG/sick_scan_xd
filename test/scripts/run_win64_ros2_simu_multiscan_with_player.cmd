REM 
REM Run sick_scan on ROS-2 Windows with multiScan pcapng player
REM 
...
rem start "ros2 sick_generic_caller" ros2 run sick_scan sick_generic_caller ./src/sick_scan_xd/launch/sick_scansegment_xd.launch hostname:=127.0.0.1 udp_receiver_ip:=127.0.0.1 publish_topic:=/cloud publish_topic_all_segments:=/cloud_360 publish_frame_id:=world add_transform_xyz_rpy:=0,0,0,0,0,0
start "ros2 sick_scan" ros2 launch sick_scan sick_scansegment_xd.launch.py hostname:=127.0.0.1 udp_receiver_ip:=127.0.0.1
...
@echo.
@echo Playing pcapng-files to emulate multiScan. Note: Start of UDP msgpacks in 20220915_multiscan_msgpack_output.pcapng takes a while...
@echo.
rem python ./src/sick_scan_xd/test/python/multiscan_pcap_player.py --pcap_filename=./src/sick_scan_xd/test/emulator/scandata/20220915_multiscan_msgpack_output.pcapng --udp_port=2115 --verbose=1
python ./src/sick_scan_xd/test/python/multiscan_pcap_player.py --pcap_filename=./src/sick_scan_xd/test/emulator/scandata/20220915_multiscan_msgpack_output.pcapng --udp_port=2115
python ./src/sick_scan_xd/test/python/multiscan_pcap_player.py --pcap_filename=./src/sick_scan_xd/test/emulator/scandata/20210929_multiscan_token_udp.pcapng --udp_port=2115
python ./src/sick_scan_xd/test/python/multiscan_pcap_player.py --pcap_filename=./src/sick_scan_xd/test/emulator/scandata/20210929_multiscan_cola-a-start-stop-scandata-output.pcapng --udp_port=2115
@timeout /t 3
popd
