...
cp -f ./src/sick_scan_xd/test/emulator/scandata/20211201_RMS_1xxx_start_up.pcapng.json /tmp/lmd_scandata.pcapng.json
sleep  1 ; ros2 run sick_scan sick_scan_emulator ./src/sick_scan_xd/test/emulator/launch/emulator_rms1xxx.launch scanner_type:=sick_rms_1xxx &
# sleep  1 ; ros2 run sick_scan sick_generic_caller ./src/sick_scan_xd/launch/sick_rms_1xxx.launch hostname:=127.0.0.1 port:=2111  & 
# sleep  1 ; ros2 run sick_scan sick_generic_caller ./src/sick_scan_xd/launch/sick_rms_1xxx.launch hostname:=127.0.0.1 port:=2111 sw_pll_only_publish:=False & 
sleep  1 ; ros2 launch sick_scan sick_rms_1xxx.launch.py hostname:=127.0.0.1 port:=2111 sw_pll_only_publish:=False &
